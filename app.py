...
@st.cache_data(ttl=300, show_spinner=False)
def get_news(ticker, limit=10, max_age_hours=72):
    """Fetch genuinely recent news for a ticker.

    The old implementation took the first RSS items returned by Google News,
    which could be stale even when newer coverage existed.  We now fetch a
    larger pool, parse publication timestamps, deduplicate headlines, and
    reject articles older than max_age_hours.
    """
    ticker = str(ticker or "").strip()
    if not ticker:
        return pd.DataFrame(columns=["Title", "Date", "Link", "Age Hours"])
    ...
    try:
        ...
        for item in root.findall(".//item"):
            ...
            date_text = (date_node.text or "").strip() if date_node is not None else ""
            ...
            try:
                published = pd.to_datetime(date_text, utc=True)
        ...
        rows.sort(key=lambda x: x["Age Hours"])
        return pd.DataFrame(rows[:int(limit)], columns=["Title", "Date", "Link", "Age Hours"])
